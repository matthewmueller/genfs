...
func wrapFile(file fs.File, genfs fs.FS, path string) *wrappedFile {
	return &wrappedFile{
		...
		offset:      0,
		readOnce:    new(onceBytes),
		readDirOnce: new(onceDirEntries),
		statOnce:    new(onceFileInfo),
	}
}
...
type wrappedFile struct {
	...
	offset      int64
	readOnce    *onceBytes
	readDirOnce *onceDirEntries
	statOnce    *onceFileInfo
}
...
func (w *wrappedFile) readDir() (des []fs.DirEntry, err error) {
...
func (w *wrappedFile) ReadDir(count int) (des []fs.DirEntry, err error) {
	...
	des, err = w.readDirOnce.Do(func() ([]fs.DirEntry, error) { return w.readDir() })
	if err != nil {
		return nil, err
	}
	offset := int(w.offset)
